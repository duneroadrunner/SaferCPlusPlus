...
#define MSE_THREADLOCAL_USING(Derived, Base) MSE_USING(Derived, Base)
	...
	private:
		...
		friend class us::impl::TCommonizedPointer<_Ty, TThreadLocalPointer<_Ty> >;
		friend class us::impl::TCommonizedConstPointer<const _Ty, TThreadLocalPointer<_Ty> >;
	};
	...
	private:
		...
		friend class us::impl::TCommonizedConstPointer<const _Ty, TThreadLocalConstPointer<_Ty> >;
	};
...
namespace std {
	template<class _Ty>
	struct hash<mse::TThreadLocalFixedPointer<_Ty> > {	// hash functor
		typedef mse::TThreadLocalFixedPointer<_Ty> argument_type;
		typedef size_t result_type;
		size_t operator()(const mse::TThreadLocalFixedPointer<_Ty>& _Keyval) const _NOEXCEPT {
	...
	template<class _Ty>
	struct hash<mse::TThreadLocalFixedConstPointer<_Ty> > {	// hash functor
		typedef mse::TThreadLocalFixedConstPointer<_Ty> argument_type;
		typedef size_t result_type;
		size_t operator()(const mse::TThreadLocalFixedConstPointer<_Ty>& _Keyval) const _NOEXCEPT {
			const _Ty* ptr1 = nullptr;
			if (_Keyval) {
				ptr1 = std::addressof(*_Keyval);
			}
...
#define MSE_DECLARE_THREAD_LOCAL(type) thread_local mse::TThreadLocalObj<type> 
#define MSE_DECLARE_THREAD_LOCAL_CONST(type) thread_local const mse::TThreadLocalObj<type> 
	...
	public:
		static void s_test1() {
			...
			public:
				...
				static int foo2(mse::TThreadLocalFixedPointer<A> A_thread_local_ptr) { return A_thread_local_ptr->b; }
			protected:
				~B() {}
			};
...

			{
				...
				mse::TThreadLocalFixedPointer<A> A_thread_local_ptr1(&thread_local_a);
				assert(A_native_ptr->b == A_thread_local_ptr1->b);
				mse::TThreadLocalFixedPointer<A> A_thread_local_ptr2 = &thread_local_a;

				/* mse::TThreadLocalFixedPointers can be coerced into native pointers if you need to interact with legacy code or libraries. */
				...
				mse::TThreadLocalFixedConstPointer<A> rcp = A_thread_local_ptr1;
				mse::TThreadLocalFixedConstPointer<A> rcp2 = rcp;
				MSE_DECLARE_THREAD_LOCAL_CONST(A) cthread_local_a(11);
				mse::TThreadLocalFixedConstPointer<A> rfcp = &cthread_local_a;
			}

			{
				...
				mse::TThreadLocalFixedPointer<GE> GE_thread_local_ifptr1 = &thread_local_gd;
				mse::TThreadLocalFixedPointer<E> E_thread_local_ifptr5 = GE_thread_local_ifptr1;
				mse::TThreadLocalFixedPointer<E> E_thread_local_fptr2(&thread_local_gd);
				mse::TThreadLocalFixedPointer<E> E_thread_local_ifptr2(&thread_local_gd);
				mse::TThreadLocalFixedConstPointer<E> E_thread_local_fcptr2 = &thread_local_gd;
			}

			{
				...
				public:
					static int foo1(A* a_native_ptr) { return a_native_ptr->b; }
					static int foo2(mse::TThreadLocalFixedPointer<A> A_scpfptr) { return A_scpfptr->b; }
					static int foo3(mse::TThreadLocalFixedConstPointer<A> A_scpfcptr) { return A_scpfcptr->b; }
				protected:
					~B() {}
				};
...

			{
				...
				MSE_DECLARE_THREAD_LOCAL(GE) thread_local_gd;
				mse::TThreadLocalFixedPointer<GE> GE_thread_local_ifptr1 = &thread_local_gd;
				mse::TThreadLocalFixedPointer<E> E_thread_local_ptr5(GE_thread_local_ifptr1);
				mse::TThreadLocalFixedPointer<E> E_thread_local_ifptr2(&thread_local_gd);
				mse::TThreadLocalFixedConstPointer<E> E_thread_local_fcptr2 = &thread_local_gd;
			}
