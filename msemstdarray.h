...
#define _NOEXCEPT
		...
		public:
			...
			void default_initialize(std::true_type) {
				mse::msearray<_Ty, _Size> ma = {};
				m_msearray =(ma);
			}
			void default_initialize(std::false_type) {
			}
			array(_XSTD initializer_list<typename _MA::base_class::value_type> _Ilist) {
				...
				if (_Size > _Ilist.size()) {
					...
					default_initialize(std::is_default_constructible<_Ty>{});
				}
				msear_size_t count = 0;
				auto Il_it = _Ilist.begin();
				auto target_it = m_msearray.begin();
