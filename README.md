...
And technically, copy and move constructors are unsafe as they take (technically unsafe) reference parameters and are non-static member functions (that provide access to the unsafe (explicit and implicit) `this` pointer). Same goes for operators. Destructors and default constructors don't have the unsafe reference parameter issue, but still have the `this` pointer issue. While these elements are technically unsafe, empirically (and perhaps intuitively) they seem to be much less prone to memory safety bugs than, say, raw pointers or "non-bounds-checked" containers. 
