...
usage example: ([link to interactive version](https://godbolt.org/z/r99osrGne))
...
As explained following, (at the time of writing) there exist situations where `TInt<>` does not fully address the problem of integer overflow. And when it does, it's not always in the absolute optimal way in terms of avoiding theoretical run-time overhead. There exist other C++ integer (replacement) implementations that address the issue of integer overflow more comprehensively. When feasable, it is recommended that you use them. The integer replacements in this library, on the other hand, might be positioned as a sometimes more practical compromise, for example, between overflow safety and run-time and compile-time overhead.

By default, the result of an arithmetic operation involving a `TInt<>` (or `CSize_t`) is returned as a (larger) (`TInt<>`) type that can accomodate the range of possible results (without risk of overflow), if such a type is available. Assigning such a result, for example, back to one of the original `TInt<>` operands may result in a "narrowing" conversion that would be checked (at runtime).

This basically means that all arithmetic operations are overflow checked when necessary except for arithmetic operations on `TInt<>`s based on the largest integer (signed and unsigned) types. But, if the largest recognized integer types are 128 bits or more, then arithmetic operations that would have returned `TInt<>`s based on those types will instead return `TInt<>`s based on the smallest type that can accommodate the ranges of all the argument types. So, for example, if the largest recognized integer type is 128 bits, a multiply operation of two `TInt<>`s based on 64-bit integers will be computed as 128-bit integers (avoiding any risk of overflow), but then cast back to a 64-bit based `TInt<>` result. That (narrowing) cast will check for overflow (at run-time). This means that as long as the original (`TInt<>`) arguments are based on 64-bit (or lower) integers, you can perform any arbitrary number of (chained) arithmetic operations without risk of unchecked overflow.

As of this writing, g++ and clang++ support 128-bit integers on 64-bit platforms as a compiler-specific extension. The library does recognize and use those. (You can prevent it from recognizing those by defining the `MSE_IMPL_DO_NOT_USE_INT128_COMPILER_EXTENSION` preprocessor macro.) The `MSE_IMPL_BITSIZE_REQUIRED_TO_RESERVE_THE_LARGEST_INTEGERS_FOR_OVERFLOW_CHECKING` preprocessor macro can be used to change the minimum size the largest recognized integers need to be in order to prevent arithmetic operators from promoting their results to `TInt<>`s based on the largest integers. (Note the value corresponds to `std::numeric_limits<>::digits` and defaults to `127`.) And finally, the promotion of arithmetic results to larger (`TInt<>`) types can be completely disabled by defining the `MSE_DISABLE_RETURN_RANGE_EXTENDED_TYPE_FOR_INTEGER_ARITHMETIC` preprocessor macro.

Microbenchmarks at the time of writing measure a run-time overhead of around 20% for addition operations between `TInt<>`s based on 64-bit signed integers (on x64 hardware compiled with clang++ (version 18.1.3)) versus 64-bit addition operations without overflow protection. This overhead is primarily due to 128-bit addition being slower than 64-bit addition (on these 64-bit systems). But note however that the corresponding overhead for unsigned integers was measured to be only around 4%. This is because clang++ is able to use the fact that they're unsigned to conclude that for branches where overflow is determined to have not occurred, such 128-bit addition operations can be replaced with equivalent (and faster) 64-bit addition operations. 
...
In contrast to `std::optional<>::value()` which returns a reference to the contained element, `std::any_cast<>()` does not return a reference. If the argument is an `std::any`, it returns a value corresponding to the contained element. If the argument is instead a (raw) pointer to an `std::any`, it returns a (possibly null) raw pointer to the contained element.

Because of safety requirements, this library's implementation of `any_cast<>()` accepts any type of (safe smart) pointer targeting any of the library's `any` types, and returns another (possibly null) safe (smart) pointer to the contained element. Additionally, the library provides a `maybe_any_cast_ptr<>()` template function which (only) accepts any type of (safe smart) pointer targeting any of the library's `any` types and returns an `optional<>` pointer to the contained element (in lieu of returning a null pointer when the cast fails), as well as `maybe_any_cast<>()` which accepts any of the library's `any` types (rather than a pointer to one of them) and returns an `optional<>` value (in lieu of throwing an exception when the cast fails).
