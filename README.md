...
You can instantiate `TInt<>`s with overflow checking of arithmetic operations by using `mse::int_options_t<mse::enable_AR_overflow_checking_t>` as the second template argument. (For example, `mse::TInt<int32_t, mse::int_options_t<mse::enable_AR_overflow_checking_t> >`.) 
